...
export function Footer() {
  ...
  return (
    <footer className="relative z-10 overflow-hidden min-h-[420px]">
      ...
      <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(38, 12, 68, 0.78) 0%, rgba(30, 9, 56, 0.88) 100%)" }} />

      <div className="relative z-10 pt-16 sm:pt-24">
        <div className="mx-auto max-w-7xl px-6 pb-8 lg:px-8">
          <div className="xl:grid xl:grid-cols-3 xl:gap-12">
            <div className="space-y-6 xl:col-span-1">
              <Link to="/" className="text-2xl font-display tracking-tight text-white hover:text-[#c4a8f0] transition-colors">Crest</Link>
              ...
              <p className="text-sm text-white/50"><a href="mailto:sales@crestsecurity.io" className="hover:text-[#c4a8f0] transition-colors">sales@crestsecurity.io</a></p>
            </div>
            ...
            <div className="mt-16 grid grid-cols-2 gap-8 xl:col-span-2 xl:mt-0">
                ...
                <div>
                  ...
                  <ul role="list" className="space-y-4">
                    {navigation.product.map((item) => (
                      <li key={item.name}><a href={item.href} onClick={(e) => scrollTo(e, item.href)} className="text-sm text-white/45 hover:text-[#c4a8f0] transition-colors">{item.name}</a></li>
                    ))}
                  </ul>
                </div>
              ...
              <div>
                ...
                <ul role="list" className="space-y-4">
                  {legal.map((item) => (
                    <li key={item.name}><Link to={item.href} className="text-sm text-white/45 hover:text-[#c4a8f0] transition-colors">{item.name}</Link></li>
                  ))}
                </ul>
              </div>
            </div>
