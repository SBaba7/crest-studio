...
//   u_shape = vec4(1.68, 0.72, 0.50, 0.00)
//   u_surface = vec4(2.40, 1.08, 0.00, 1.00)
//   u_finish = vec4(0.00, 0.00, 0.000, 0.08)
//   u_transform = vec4(8617.0, 0.00, 0.00, 0.0)
...
export function PlasmaShader({ className }: PlasmaShaderProps) {
  ...
  useEffect(() => {
    ...
    // Plain WebGL1 context, no libraries
    ...
    // Colors: #02010A, #04052E, #3D2C8D, #916BBF (8 colors total, first 4 active)
    ...
    // Static uniform vectors
    const shape = new Float32Array([1.68, 0.72, 0.50, 0.00]);
    const surface = new Float32Array([2.40, 1.08, 0.00, 1.00]);
    const finish = new Float32Array([0.00, 0.00, 0.000, 0.08]);
    const transform = new Float32Array([8617.0, 0.00, 0.00, 0.0]);
    const space = new Float32Array([0.00, 0.00, 0.0, 0.0]);
    const cursor = new Float32Array([0.0, 2.0, 0.65, 0.46]); // Cursor: off (presence: 0.0)
    ...
    const render = (now: number) => {
      ...
      // Upload packed uniforms
      gl.uniform3fv(uColorsLoc, colors);
      gl.uniform4f(uSceneLoc, width, height, accumulatedTime * 0.76, 4.0);
      gl.uniform4fv(uShapeLoc, shape);
      gl.uniform4fv(uSurfaceLoc, surface);
      gl.uniform4fv(uFinishLoc, finish);
      gl.uniform4fv(uTransformLoc, transform);
      gl.uniform4fv(uSpaceLoc, space);
      gl.uniform4fv(uCursorLoc, cursor);
