...
// "Simplex Noise" — made with the 21st.dev Shader Builder
...
vec3 linToOklab(vec3 c) {
  ...
  return vec3(
    ...
    0.0259040371 * l + 0.7827717662 * m - 0.8086757662 * s);
}
...
export function SimplexNoiseWebGL({ className = "" }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const render = (now: number) => {
        ...
        gl.uniform4f(finish, 0.0, 0.0, 0.0, 0.04);
      ...
      const handleVisibility = () => {
        ...
        if (running && animationFrame === 0) animationFrame = window.requestAnimationFrame(render);
      ...
      console.error("SimplexNoiseWebGL initialization failed", error);
    }
  }, []);
...
export default SimplexNoiseWebGL;
